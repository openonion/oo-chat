...
describe('live release evidence helpers', () => {
  ...
  it('uses non-blocking client navigation and bounded owned-browser cleanup', () => {
    ...
    expect(runner).toContain('wait_for_claude_parent_complete 300')
    ...
    expect(runner).toContain("'s/.*\"messageCount\":[[:space:]]*\\([0-9][0-9]*\\).*/\\1/p'")
    expect(runner).toContain('[[ -n "$message_count" && "$message_count" -ge 2 ]]')
    expect(runner).not.toContain("'\"messageCount\":[[:space:]]*[2-9][0-9]*'")
