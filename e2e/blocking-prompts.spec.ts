 ...
 * has hit its Host-owned turn limit. The browser may end it, but cannot mint more
 * authority or extend the budget on its own.
 */
...
test.describe('an autonomous run hits its limit', () => {
  test('says how far it got and lets the reader end, not extend, the run', async ({ page, shot }) => {
    await run(page, 'full-access-checkpoint')

    // How far it got, against what the Host allowed. A missing max_turns used
    // to render "20 of 0 turns".
    await expect(page.getByText('Completed 20 of 100 turns')).toBeVisible({ timeout: 20_000 })
    await expect(page.getByRole('button', { name: /end full access run/i })).toBeVisible()
