...
export async function mockAgent(
  ...
  await page.routeWebSocket(url => !url.pathname.includes('_next'), ws => {
    ...
    ws.onMessage(raw => {
      ...
      if (scenario === 'coding-agent-claude') {
        send(ws, {
          type: 'tool_call', id: 'call-8', name: 'claude_code',
          args: { prompt: 'Inspect the reconnect boundary and explain the next step.' }, status: 'running',
        })
        send(ws, {
          ...
          permissionMode: 'default', sessionId: 'claude-session-1', status: 'running',
