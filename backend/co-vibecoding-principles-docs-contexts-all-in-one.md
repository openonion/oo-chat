...
- Activity logging to .co/logs/ (Python SDK only) - See "Logging & Debugging" section
- Interactive debugging with @xray and agent.auto_debug() - See "Interactive Debugging" section
...
ConnectOnion automatically logs all agent activity to help you debug, audit, and understand your agent's behavior.

### Automatic Logging to .co/logs/

By default, all agent activity is logged to `.co/logs/{agent_name}.log`:

```python
from connectonion import Agent

agent = Agent("my-agent", tools=[search, calculate])
agent.input("Find the answer to 2+2")

# Logs written to: .co/logs/my-agent.log
```

**Log file contains:**
- Timestamps for each operation
- User input prompts
- LLM calls with model and timing
...
```
[2025-01-20 10:30:15] User: Find the answer to 2+2
[2025-01-20 10:30:15] LLM Call (gpt-4-mini) - 234ms
[2025-01-20 10:30:16] Tool: calculate(expression="2+2") -> 4
[2025-01-20 10:30:16] Tool execution: 45ms
[2025-01-20 10:30:17] LLM Call (gpt-4-mini) - 189ms
[2025-01-20 10:30:17] Response: The answer is 4
[2025-01-20 10:30:17] Total duration: 2.1s
```
...
**Disable logging:**
```python
agent = Agent("my-agent", tools=[search], log=False)
```

**Log to current directory:**
```python
agent = Agent("my-agent", tools=[search], log=True)
# Logs to: ./my-agent.log
```

**Custom log file path:**
```python
agent = Agent("my-agent", tools=[search], log="logs/my-custom-log.log")
```

**Environment variable (highest priority):**
```bash
export CONNECTONION_LOG=/var/log/my-agent.log
```

### Accessing Activity History Programmatically

**Access session trace:**
```python
from connectonion import Agent
...
for entry in trace:
    if entry['type'] == 'tool_execution':
        ...
        print(f"Timing: {entry['timing']}ms")
```
...
tail -n 50 .co/logs/my-agent.log
```

**2. Performance analysis:**
```bash
# Find slow tool calls
grep "Tool execution" .co/logs/my-agent.log | grep -E "[0-9]{4,}ms"
```

**3. Audit trail:**
```python
# All agent actions are logged for compliance
agent = Agent("finance-agent", tools=[transfer_money])
agent.input("Transfer $1000 to account XYZ")
# Logged: timestamp, tool call, arguments, result
```

**4. Testing and monitoring:**
```python
# Check if agent used expected tools
trace = agent.current_session['trace']
tool_names = [t['tool_name'] for t in trace if t['type'] == 'tool_execution']
assert 'validate_account' in tool_names
```

### Combining Logging with Debugging

Use both logging and interactive debugging together:

```python
from connectonion import Agent
from connectonion.decorators import xray

@xray  # Breakpoint for interactive debugging
def critical_operation(data: str):
    """This tool will pause execution for inspection."""
    return process(data)

# Enable both logging and debugging
agent = Agent("dev-agent", tools=[critical_operation], log=True)
agent.auto_debug()  # Interactive debugging

# Now you get:
# - Log files for historical analysis
# - Interactive breakpoints for live debugging
agent.input("Process important data")
```

### Best Practices

1. **Keep logs enabled in development** - Free debugging help
2. **Use custom paths in production** - Centralized logging
3. **Rotate logs regularly** - Prevent disk space issues
4. **Check logs first when debugging** - Often shows the problem immediately
5. **Combine with @xray for live debugging** - Best of both worlds
