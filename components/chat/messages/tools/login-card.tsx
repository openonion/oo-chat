...
import { HiOutlineLockClosed, HiOutlineCheck, HiOutlinePencilAlt } from 'react-icons/hi'
...
// Verification / 2FA codes are part of the login flow: same secure modal, kept out
// of the transcript — but shown as plain text (not masked), since you read the code.
function isCodeField(f: { name: string; label?: string }): boolean {
  return /verif|otp|2fa|one.?time|sms|passcode|\bcode\b/i.test(`${f.name || ''} ${f.label || ''}`)
}
...
export function LoginCard({ toolCall, pendingAskUser, onAskUserResponse }: LoginCardProps) {
  ...
  // This card renders any structured ask_user (a field form returning JSON), not
  // only logins. Show the secure login chrome (lock, kept out of transcript) for
  // credentials — passwords AND verification codes — otherwise a neutral input
  // prompt (e.g. post text). Fall back to args.fields so detection holds once
  // pendingAskUser clears.
  const argFields = (args?.fields as Array<{ name: string; type?: string; label?: string }> | undefined) ?? []
  const allFields = fields ?? argFields
  const hasPassword = allFields.some(isPasswordField)
  const hasCode = allFields.some(isCodeField)
  const isLogin = hasPassword || hasCode
  const title = hasPassword ? 'Sign in' : hasCode ? 'Verification' : 'Your response'
  const submitLabel = hasPassword ? 'Sign in' : hasCode ? 'Verify' : 'Submit'
        ...
        <div className={`flex items-center justify-center w-5 h-5 rounded-full ${done ? 'bg-green-100/60' : 'bg-neutral-100'}`}>
          {done
            ...
            : (isLogin
                ...
                : <HiOutlinePencilAlt className="w-3 h-3 text-neutral-500" />)}
        </div>
        <span className="font-medium text-neutral-600">
          {done
            ? (hasPassword ? 'Credentials provided' : hasCode ? 'Code submitted' : 'Response submitted')
            : (hasPassword ? 'Sign-in requested' : hasCode ? 'Verification requested' : 'Input requested')}
        </span>
      </div>
